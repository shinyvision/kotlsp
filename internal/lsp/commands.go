...
func (s *Server) exportWorkspace(args []json.RawMessage) (any, *jsonrpc.ResponseError) {
	if len(args) != 1 {
	...
	dependencies := make([]map[string]any, 0, len(classpath)+2)
	dependencies = append(dependencies, map[string]any{"type": "moduleSource"}, map[string]any{"type": "inheritedSdk"})
	...
	modules := make([]map[string]any, 0, len(roots))
	for _, rootURI := range roots {
		root, ok := uriutil.Path(rootURI)
		if !ok {
			continue
		}
		sourceRoots := make([]map[string]any, 0)
		for _, candidate := range []struct{ path, kind string }{
			{"src/main/kotlin", "java-source"}, {"src/main/java", "java-source"},
			{"src/test/kotlin", "java-test"}, {"src/test/java", "java-test"},
		} {
			path := filepath.Join(root, filepath.FromSlash(candidate.path))
			if stat, statErr := os.Stat(path); statErr == nil && stat.IsDir() {
				sourceRoots = append(sourceRoots, map[string]any{"path": path, "type": candidate.kind})
			}
		}
		modules = append(modules, map[string]any{
			"name": filepath.Base(root), "type": "JAVA_MODULE", "coordinate": nil,
			"dependencies": dependencies,
			"contentRoots": []any{map[string]any{"path": root, "excludedPatterns": []string{}, "excludedUrls": []string{}, "sourceRoots": sourceRoots}},
			"facets":       []string{}, "externalProjectPath": root,
		})
	}
	s.rootMu.RLock()
	...
	workspace := map[string]any{
		...
		"kotlinSettings": []any{}, "javaSettings": []any{},
	}
	...
	if err = os.WriteFile(filepath.Join(destination, "workspace.json"), payload, 0o600); err != nil {
